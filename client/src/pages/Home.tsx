...
const Home = () => {
  ...
  useEffect(() => {
    async function loginTelegram() {
      ...
      let invite_by = '';
      let avatar = '';
      // Ưu tiên lấy từ Telegram WebApp
      if (window.Telegram && window.Telegram.WebApp && window.Telegram.WebApp.initDataUnsafe.user) {
        telegram_id = window.Telegram.WebApp.initDataUnsafe.user.id.toString();
        username = window.Telegram.WebApp.initDataUnsafe.user.username || '';
        // Lấy avatar Telegram (dạng https://t.me/i/userpic/320/{telegram_id}.jpg)
        avatar = `https://t.me/i/userpic/320/${telegram_id}.jpg`;
        // Nếu có start_param (invite), lấy làm invite_by
        if (window.Telegram.WebApp.initDataUnsafe.start_param) {
          invite_by = window.Telegram.WebApp.initDataUnsafe.start_param;
        }
      } else {
        // Fallback: lấy từ localStorage hoặc query string
        telegram_id = localStorage.getItem('telegram_id') || '';
        username = localStorage.getItem('username') || '';
        invite_by = localStorage.getItem('invite_by') || '';
      }
      if (telegram_id) {
        // Gửi lên backend để login/khởi tạo user
        const res = await fetch('https://pxiltsic.cloudfly.vn/api/login', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ telegram_id, username, invite_by, avatar })
        });
  ...
  try {
    const response = await fetch('https://pxiltsic.cloudfly.vn/api/open-egg', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    // Gọi API thưởng cho inviter (nếu có)
    if (user) {
      try {
        const res = await fetch('https://pxiltsic.cloudfly.vn/api/open-chest', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          ...
          <div>
            <div className="flex items-center">
              <span className="text-sm font-medium">
                {user?.username ? `@${user.username}` : user?.telegram_id || 'Player'}
              </span>
            ...
            <div className="px-2 py-0.5 bg-yellow-500/20 text-yellow-500 text-xs rounded-full inline-flex items-center space-x-1">
              ...
              <span className="text-[10px] font-medium">{userData?.openedEggs || 0} {t('eggsOpened')}</span>
            </div>
          </div>
        </div>
        <div className="flex items-center">
              ...
              feCollection.map((kitty) => (
                <div key={kitty.id} className="flex flex-row items-center bg-gray-800/70 rounded-lg p-2 border border-primary/10 min-w-[140px] relative">
                  <div className="w-10 h-10 mr-2 rounded-full border-2 border-yellow-400 bg-black flex items-center justify-center shadow-lg overflow-hidden relative">
                    <img src={kittyImages[kitty.name]} alt={kitty.name} className="w-full h-full rounded-full object-cover" style={{imageRendering: 'auto'}} />
                  </div>
